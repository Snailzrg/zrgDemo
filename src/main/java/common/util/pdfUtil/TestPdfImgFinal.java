...
//                htmlFile.delete();
    ...
    private String initHtmlStr(final Map map, final String qrStr) {
        final StringBuilder sb = new StringBuilder();
        sb.append("@page {size: 842px*595px}table td ");
        sb.append("{text-align: center;width: 15%;height: 56px;font-size: 14px;}");
        sb.append("");
        sb.append("");  // 替换html中图片（虽然现在用不到）
        sb.append("");
        sb.append("国网内蒙古东部电力有限公司粘贴单");  // table
        sb.append("");
        sb.append("");
        sb.append("单位:" + getMapItemVal(map, "dwdh") + "");
        sb.append("");
        sb.append(getMapItemVal(map, "zdrq") + "单位:元\n");  // 表格 单据编号
        sb.append("");
        sb.append("单据编号:");
        sb.append("" + getMapItemVal(map, "billid") + "");  //
        sb.append("费用类形:" + getMapItemVal(map, "FYLXQZBDYH") + "附件张数:");
        sb.append(map.get("FDZS") + "部门:" + getMapItemVal(map, "SSBM") + "");
        sb.append("金额:" + getMapItemVal(map, "SUMMONEY") + "经办人:" + getMapItemVal(map, "TRANSACTOR") + "证明人:");  //
        sb.append("  ");
        return sb.toString();
    }

    /**
     * .  *   * @param map  *            map  * @param key  *            key  * @return str
     */
    private String getMapItemVal(final Map<String,String> map, final String key) {
     ...
     * @return str
     */
    private static String initHtmlStr(final Map<String, String> map, final String qrStr, final double weight,
                                      final double height) {
        final StringBuilder sb = new StringBuilder();
        sb.append("<html>\n" +
                "\t<head>\n" +
...
//   "+qrStr+"
    ...
    private static void writeStringToFile(final File file, final String data, final String encoding) throws
            IOException {
    ...
    // 設置A4
    private static void htmlToPdf(final String htmlPath, final String pdfPath, String qrStr) throws

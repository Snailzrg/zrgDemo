...
public class JsonUtil {
    ...
    public static <T> T jsonToBean(String jsonString, Class<T> beanCalss) {
		...
		T bean = (T) JSON.toJavaObject(jsonObject,beanCalss.getClass());
        return bean;
    }
    ...
    public static String beanToJson(Object bean) {
    	JSONObject json = JSONObject.parseObject(bean.toString());
    ...
    public static String beanToJson(Object bean, String[] _nory_changes, boolean nory) {
    	...
    	//	json = JSONObject.parseObject(bean, configJson(str.split(":")));
    	}else{//转换除了_nory_changes里的属性
    	//	json = JSONObject.parseObject(bean,configJson(_nory_changes));
    	}
    	return json.toString();
	}
//
...
//     }
    ...
    public static <T> List<T> jsonToBeanList(String jsonString, Class<T> beanClass) {
    	...
    	T bean;
    	int size = jsonString != null ? jsonArray.size() : 0;
    	List<T> list = new ArrayList<T>(size);
    	for (int i = 0; i < size; i++) {
			jsonObject = jsonArray.getJSONObject(i);
			bean = (T)	JSON.toJavaObject(jsonObject,beanClass.getClass());
    		list.add(bean);
    	}
    	return list;
    }
